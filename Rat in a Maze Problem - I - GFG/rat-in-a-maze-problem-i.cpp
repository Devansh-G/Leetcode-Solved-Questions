...
class Solution{
    ...
    //Without DFS, and instead using visited array
    
    void fun(vector<string> &v, vector<vector<int>> &vis, string &s, int i, int j, vector<vector<int>> & arr, int &n)
    {
        if(i==n-1 && j==n-1)
        {
            v.push_back(s);
        ...
        {
            int nx = i+ ax[k] , ny = j + ay[k];
            if(nx>=0 && nx<n && ny>=0 && ny <n && arr[nx][ny]==1 && vis[nx][ny]==0)
            {
                vis[nx][ny]=1;
                s+=letter[k];
                fun(v,vis,s,nx,ny,arr,n);
                vis[nx][ny]=0;
                s.pop_back();
            }
        }
